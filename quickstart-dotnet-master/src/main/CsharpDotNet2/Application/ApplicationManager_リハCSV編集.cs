    ...
    class ApplicationManager
    {
        ...
        public string actionSubscrWithCsv(string prodId, string[] readedCsvCol, string[] readedCsvColHeader)
        {
            try
            {
                ...
                // 2.明細CSVの数量が２以上はDBAなので子品目の数量に設定する。
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (planCount > 1)
                        {
                            ...
                            {
                                localVarResponse = actionCreateApi.ProxyActionPOSTsubscribe(subscribeRequest);
                                Console.Out.WriteLine("ProxyActionPOSTsubscribe（PlanQuantity__c）: " + localVarResponse.ToString());
                            }
                        }
                        //ゼロ対応：ゼロ入力はいたずら？                    
                        if (planCount > 0)
        ...
        public PUTSubscriptionResponseType updateOyaSubscriptionNo(string[] readedCsvCol)
        {
            ...
            {                
                ...
                while (!parser.EndOfData)
                {
                    ...
                    /// 29_QuoteNumber__QT、49_ParentCode__c、52_ProductCode__c、8_subscriptionNumber
                    /// 
                    if (readedCsvCol[29].Equals(csvDB[29]))    //csv.資産番号 = DB.資産番号
        ...
        public PUTSubscriptionResponseType updateSubscrWithCsv(string[] readedCsvCol)
        {
            try
            {
                ...
                */
                ...
                // 31_ParentSubscriptionNumber__c
                if (!string.IsNullOrEmpty(readedCsvCol[31]))
                    subscription.ParentSubscriptionNumber__c = readedCsvCol[31];
                // 35_AutoRenewVersion__c
                if (!string.IsNullOrEmpty(readedCsvCol[35]))
                    subscription.AutoRenewVersion__c = readedCsvCol[35];
                // 36_ServiceStartDate__c //なぜかDateTimeではエラーになるので、文字列の"YYYY-MM-DD"とする
                ...
                if (!string.IsNullOrEmpty(readedCsvCol[41]))
                    subscription.NeTTSSyncFlag__c = readedCsvCol[41];
                ...
                */
                // subscription ratePlanID(44_id -> 43_id)
                ratePlan.RatePlanId = readedCsvCol[43];
                //11_contractEffectiveDate
                if (!string.IsNullOrEmpty(readedCsvCol[11])) 
                ...
                ratePlanCharge.RatePlanChargeId = readedCsvCol[56];  // ratePlanChargeId (55_id -> 56_id)

                if (!string.IsNullOrEmpty(readedCsvCol[69]))                
                    ratePlanCharge.Price = readedCsvCol[69]; //68_price -> 69_price                

                //■●★ フィールド「quantity」は課金モデルがFlat Fee Pricingの課金に無効です
                // 60->61_type =Recurring  61->62_model = PerUnit のみ数量変更可能
                //if (!readedCsvCol[61].Equals("FlatFee"))
                if (readedCsvCol[61].Equals("Recurring") && readedCsvCol[62].Equals("PerUnit"))                
                    if (!string.IsNullOrEmpty(readedCsvCol[84]))
                        ratePlanCharge.Quantity = readedCsvCol[84]; //83->84_quantity

                // 66_priceChangeOption
                if (!string.IsNullOrEmpty(readedCsvCol[66]))
                    ratePlanCharge.PriceChangeOption = readedCsvCol[66];
                // 67_priceIncreasePercentage
                if (!string.IsNullOrEmpty(readedCsvCol[67]))
                    ratePlanCharge.PriceIncreasePercentage = readedCsvCol[67];
                // 83_billingPeriodAlignment
                if (!string.IsNullOrEmpty(readedCsvCol[83]))
                    ratePlanCharge.BillingPeriodAlignment = readedCsvCol[83];
                // 71_includedUnits
                if (!string.IsNullOrEmpty(readedCsvCol[71]))
                    ratePlanCharge.IncludedUnits = readedCsvCol[71];
                // 72_overagePrice
                if (!string.IsNullOrEmpty(readedCsvCol[72]))
                    ratePlanCharge.OveragePrice = readedCsvCol[72];
                // 97_triggerDate
                if (!string.IsNullOrEmpty(readedCsvCol[97]))
                    ratePlanCharge.TriggerDate = manager.convertToDate(readedCsvCol[97]);
                
                // < UCE > ContractEffective, < USA > ServiceActivation,< UCA > CustomerAcceptance , < USD > SpecificDate                
                // 98_triggerEvent
                if (!string.IsNullOrEmpty(readedCsvCol[98]))
                {
                    ...
                    else if (readedCsvCol[98].Equals("SpecificDate"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(readedCsvCol[97]))
                        {
                            DateTime dateCsv = DateTime.Parse(readedCsvCol[97]);
                ...
                if (!string.IsNullOrEmpty(readedCsvCol[107]))
                    ratePlanCharge.Description = readedCsvCol[107];
                // 108_UserPrice__c
                if (!string.IsNullOrEmpty(readedCsvCol[108]))
                    ratePlanCharge.UserPrice__c = readedCsvCol[108];
                ...
                if (!string.IsNullOrEmpty(readedCsvCol[70]))  // 69->70_tiers=nullなら 112->110_tierのデータはない。。。
                {
                    ...
                    */
                    ...
                    for (int i = 0; (110 + 6 * i) < lengthCsvCol; i++ ) { 
                        //Initialize the Rate Plan CHARGE container list (Must use a list as the subscription can have multiple rate plans)
                        //List<POSTTierType> ratePlanChargeTierList = new List<POSTTierType>();
                        //Initialize the individual Rate Plan container
                        POSTTierType ratePlanChargeTier = new POSTTierType();
                        //Populate the Rate Plan container with the required field                
                        if (!string.IsNullOrEmpty(readedCsvCol[110 + 6 * i]))   //112_tier -> 110_tier
                            ratePlanChargeTier.Tier = long.Parse(readedCsvCol[110 + 6 * i]);                                       
                        if (!string.IsNullOrEmpty(readedCsvCol[111 + 6 * i]))   //113_startingUnit -> 111_startingUnit
                            ratePlanChargeTier.StartingUnit = readedCsvCol[111 + 6 * i];
                        if (!string.IsNullOrEmpty(readedCsvCol[112 + 6 * i]))  //114_endingUnit -> 112_endingUnit
                            ratePlanChargeTier.EndingUnit = readedCsvCol[112 + 6 * i];
                        if (!string.IsNullOrEmpty(readedCsvCol[113 + 6 * i]))  //115_price -> 113_price
                            ratePlanChargeTier.Price = readedCsvCol[113 + 6 * i];
                        if (!string.IsNullOrEmpty(readedCsvCol[114 + 6 * i]))  //116_priceFormat -> 114_priceFormat
                            ratePlanChargeTier.PriceFormat = readedCsvCol[114 + 6 * i];
     ...
     　　　　　    */
        ...
        public void createSubscription()
        {
            ...
            while (!parserHeader.EndOfData)
            {
                ...
                //Console.Out.WriteLine("header-" + countCsvHeader + "件目");
                ...
                while (!parser.EndOfData)
                {
                    ...
                    //HeaderList                    
                    for (int nh = 0; nh < csvColumnHeader.Length; nh++)
                    {
                        Console.Write(csvColumnHeader[nh] + ",");
                    }
                    Console.WriteLine();
                    break;

                    //// 明細のLISTはEXCELにて
                    //// 10_契約終了日 ＞ 2017/10/1、 11_解約日 : 未設定
                    //DateTime endDate = DateTime.Parse(csvColumn[10]);
                    //if (string.IsNullOrEmpty(csvColumn[11]) && endDate > DateTime.Parse("2017-10-01"))
                    //{
                    //    //Header
                    ...
                    //    for (int nd = 0; nd < csvColumn.Length; nd++)
                    //    {
                    //        Console.Write(csvColumn[nd] + ",");
                    //    }
                    //    Console.WriteLine();
                    //}
                    //else
                    //{
                    //    Console.Out.WriteLine("10_契約終了日" + endDate + "11_解約日" + csvColumn[11] );
                    //}                    
                }
                //◀◀◀ End of detail CSV
        ...
        /** #2 create susscription BK
         * 
         */
        public void createSubscriptionBK()
        {
            //★ Print Log Out
            // 出力先ファイル名 subscMakeLog   追加書き込み true   文字コードSystem.Text.Encoding.GetEncoding("UTF-8")
            StreamWriter sw = new StreamWriter(subscMakeLog, true, System.Text.Encoding.GetEncoding("UTF-8"));
        ...
        */
        public void updateOya()
        {
            ...
            while (!parser.EndOfData)
            {
                ...
                if (!string.IsNullOrEmpty(csvColumn[49]))
                {
